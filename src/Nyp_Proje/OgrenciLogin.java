...
public class OgrenciLogin extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					OgrenciLogin frame = new OgrenciLogin();
	...
	public OgrenciLogin() {
		...
		JLabel idLabel = new JLabel("Öğrenci No:");
		...
		girisButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String okulNo = textField.getText();
                String sifre = textField_1.getText();
                
                if (studentDatabase.login(okulNo, sifre)) { 
                    JOptionPane.showMessageDialog(contentPane, "Giriş Başarılı!");
                    
                    Student loggedInStudent = studentDatabase.getStudentById(okulNo);

                    OgrenciSistemi ogrenciSistemi = new OgrenciSistemi(loggedInStudent);
                    ogrenciSistemi.setVisible(true);
                    dispose(); 
                } else {
		...
		JLabel lblNewLabel_1 = new JLabel("Öğrenci Girişi");
